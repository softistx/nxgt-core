...
| **Who is calling** | `@nxgt/ory-sdk`'s `resolve(headers)`: a Kratos cookie or an `X-Session-Token` → `whoami`. Either way an `OryPrincipal` whose **`subject`** is the Kratos identity id. One person is one string, whichever transport. A machine caller bearing an OAuth2 token is not this plugin's subject — see `references/oauth2-and-hydra.md`. |
