...
- Eight packages: five standalone, then a bridge on two, a kit on one, and a kit on four. `@nxgt/drizzle/pg`, `@nxgt/mongo/migrations` and `@nxgt/mongo/gridfs` are subpaths, not packages; `verify:artifacts` loads **eleven** subpaths in all.
