...
`auth: { token, refresh }` sends an awaited bearer token, and on a 401 refreshes once for every call refused together, then sends each again. `retry` sends an idempotent request again after a network failure or a 408, 429, 502, 503 or 504, honoring `Retry-After`. `use` takes middleware around each request.
