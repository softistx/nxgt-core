...
function Calendar({
	...
	return (
		<DayPicker
			...
			className={cn(
				'bg-background w-min max-w-full group/calendar p-3 [--cell-size:--spacing(8)] [[data-slot=card-content]_&]:bg-transparent [[data-slot=popover-content]_&]:bg-transparent',
			...
			components={{
				...
				WeekNumber: ({ children, ...props }) => {
					return (
						<td {...props}>
							<div className="flex size-(--cell-size) center text-center">
								{children}
							</div>
						</td>
					);
...
function CalendarDayButton({
	...
	modifiers,
	...props
}: Omit<ComponentProps<typeof DayButton>, 'color'>) {
	const defaultClassNames = getDefaultClassNames();
